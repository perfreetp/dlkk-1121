...
const ensureQueueOrder = (downloads: DownloadRecord[]): DownloadRecord[] => {
  let changed = false;
  downloads.forEach((d, i) => {
    if (d.queueOrder === undefined) {
      d.queueOrder = i;
      changed = true;
    }
  });
  if (changed) {
    fs.writeFileSync(DATA_PATH, JSON.stringify(downloads, null, 2), 'utf-8');
  }
  return downloads;
};
...
router.get('/', (_req: Request, res: Response) => {
  ...
  active.sort((a, b) => a.queueOrder - b.queueOrder);
...
router.post('/', (req: Request, res: Response) => {
  const downloads = getDownloads();
  const maxOrder = downloads.reduce((max, d) => Math.max(max, d.queueOrder || 0), -1);
  ...
  downloads.forEach(d => { d.queueOrder = (d.queueOrder || 0) + 1; });
...
router.post('/reorder', (req: Request, res: Response) => {
  ...
  downloads.forEach(d => {
    if (idOrder[d.id] !== undefined) {
      d.queueOrder = idOrder[d.id];
    }
  });
  saveDownloads(downloads);
